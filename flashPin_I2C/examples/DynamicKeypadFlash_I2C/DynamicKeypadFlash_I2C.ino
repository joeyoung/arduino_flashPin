/* @file DynamicKeypadFlash_I2C - G. D. (Joe) Young April 28, 2020
 *  revision to illustrate use of flashPin for state indicator(s)
...
|| #
     ...
     Using flashPin_I2C library for indicators
*/
...
//create a new Keypad
Keypad_I2C keypad(keypadMap, rowPins, colPins, sizeof(rowPins), sizeof(colPins), I2CADDR, PCF8575);

#define ledPin 7      // needed for flashPin constructor
flashPin_I2C myPin( ledPin, HIGH, 100, I2CADDR, PCF8575 );    // exp port pin to show alpha mode
flashPin_I2C pin12( 12, HIGH, 300, I2CADDR, PCF8575 );        // exp port pin - see HOLD below
flashPin pin8( 8, LOW, 100 );                           // arduino board pin - flash while alpha

unsigned long startTime;
//const byte ledPin = 7;	                      // Use the LED on pin 13.

void setup() {
    Serial.begin(9600);
    while( !Serial ){/*wait*/}    // needed for port-switching boards - MKR
    Wire.begin( );
//    myPin.begin( );     // OK, but not needed
//    pin12.begin( );
    keypad.begin( );
//    myPin.pin_mode(ledPin, OUTPUT);           // OK, but not needed - constructor sets up
//    myPin.pin_write(ledPin, HIGH );               // Turns the LED off.
//    pin11.pin_mode(11, OUTPUT);
//    pin11.pin_write(11, HIGH );                   // Turns the LED off.
...
//		startTime = millis();
//    }
    if( alpha ) pin8.flashRep( 1 );   // (with flashOff) replaces timer to flash arduino pin
    if( alpha ) myPin.flashRep( 1 );
    myPin.flashOff( );
    pin8.flashOff( );                 // call flashOff at loop rate - implements timers
    pin12.flashOff( );
}
